...
def overlay_ellipses(bfinder, image, firstn, color=(0, 0, 255), linewidth=2):
    # overlay ellipses
    for i, ell in enumerate(bfinder.filtered_contours):
        if i > firstn:
            break
        cv2.ellipse(image, (ell.center, ell.size, ell.angle), color, linewidth)
...
def original_image(bfinder, scale=0.2,
    ...
    if isinstance(bfinder, BlobFinder):
        raise TypeError('bfinder should be BlobFinder type')
    im2display = bfinder.original_image
